...
Input data: event-style historical CSVs produced by your pipeline:
  - historical_etf_yields_us.csv
  - historical_etf_yields_canada.csv
Columns expected: "Ticker","Ex-Div Date","Dividend","Price on Ex-Date",...

Usage:
  python plot_ticker_graph.py <TICKER> [--currency USD|CAD] [--outdir graphs]

Notes:
  - Currency is inferred from ticker suffix (.TO/.NE/.V/.CN => CAD; else USD), but you can override with --currency.
  - Output is saved to <outdir>/<TICKER>_distribution_analysis.png
"""
...
def infer_currency_from_symbol(s: str) -> str:
    up = s.upper()
    return "CAD" if up.endswith(CA_SUFFIXES) else "USD"
...
def _read_histories() -> pd.DataFrame:
    ...
    if "Ticker" not in df.columns or "Ex-Div Date" not in df.columns:
...
def plot_distribution_analysis(ticker: str, outdir: Path, currency: str | None = None) -> Path:
    ...
    # normalize ticker & attempt CA fallback if no rows
    ...
    # absolute yield % (per event)
    with np.errstate(divide="ignore", invalid="ignore"):
        yld_pct = (div / price.replace(0, np.nan)) * 100.0

    # currency inference/override
    cur = (currency or infer_currency_from_symbol(symbol)).upper()
    ...
    # ----- figure layout -----
    # Large canvas keeping ~2:1 plot area; extra height for legend/title/timestamp.
    ...
    ax_price.plot(dates, price, color=RED, linewidth=2.2, label=price_label)
    ...
    ax_div.bar(dates, div, width=bar_w, alpha=0.35, color=BLUE, label="Dividends", align="center")
    ...
    ax_div.spines.right.set_position(("axes", 1.05))  # slightly inward so all axes fit
    ...
    ax_tr.plot(tr_untaxed.index, tr_untaxed.values, color=GREEN, linestyle="-", linewidth=2.6, label="Total Return of $10,000")
    ax_tr.plot(tr_tfsa.index,    tr_tfsa.values,    color=GREEN, linestyle="-", linewidth=1.4, label="Total Return of $10,000 less 15% US Witholding Tax")
    ...
    if not ann_untaxed.empty:
        ax_ann.plot(ann_untaxed.index, ann_untaxed.values, color=LIGHT_GREEN, linestyle="--", linewidth=2.6, label="Total Annualized Return (%)")
    if not ann_tfsa.empty:
        ax_ann.plot(ann_tfsa.index, ann_tfsa.values, color=LIGHT_GREEN, linestyle="--", linewidth=1.4, label="Total Annualized Return less 15% US Witholding Tax (%)")
    ...
    ax_yld.plot(yld_pct.index, yld_pct.values, color=DARK_ORANGE, linestyle="--", linewidth=2.0, label="Yield on Ex-Date Price (%)")
    ...
    # Title, x-axis label, timestamp
    title = f"{symbol} Distribution Analysis"
    ax_price.set_title(title, fontsize=16)
    ax_price.set_xlabel("Date")
    # x-limits tightly around data with small padding to keep baseline aligned and prevent 1970 issues
    ...
    # timestamp stamped consistently bottom-right
...
def main():
    ...
    ap.add_argument("--currency", choices=["USD","CAD"], help="Override inferred currency label.")
